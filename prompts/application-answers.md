...
> **Governance:** `governance/FACT_RULES.md` — do not add facts not on the resume.  
> **Voice:** `reference/humanizer.md` — sound like a person typed this, not a cover letter bot.
...
```
I'm finishing my MS in CS at ASU in May 2026 and looking for backend-heavy SWE roles. Last year I interned at eInfochips building REST APIs with OAuth and JWT for accounting workflows. Recently I built a distributed caching system in Go that handled about 10,600 reads/sec in load tests and cut LLM prefix cost in a RAG simulation. I'm interested in Acme because your platform team works on the same async, high-throughput problems I've been practicing on personal projects.
```

**Why are you a good fit?** (~65 words)

```
This role asks for Node services and background jobs, which matches the SEO audit engine I built: Express API returns in under 400ms while BullMQ workers run Playwright crawls in the background. I also have production-style API auth from my eInfochips internship. I'm used to pairing with seniors, taking code review feedback, and shipping with CI. That lines up with your emphasis on ownership and cross-functional work with product.
```
...
- Resume tailoring: `gpt/system-prompt.md` · `governance/OUTPUT_CONTRACT.md`
- LinkedIn outreach: `prompts/linkedin-connection-note.md`
