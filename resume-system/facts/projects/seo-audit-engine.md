...
1. Built an audit submission flow and report dashboard with JavaScript, HTML, CSS, REST APIs, and Cloudflare Pages, designed to give small agencies one consistent website assessment for sales and onboarding without switching among separate SEO, performance, and accessibility tools.

2. Built asynchronous audit processing with Node.js, Express, BullMQ, Redis, and PostgreSQL, keeping submissions under 400 ms while Playwright crawls ran in the background so one slow site could not block the remaining batch.

3. Automated service deployments with Git and CI/CD, designed to keep the audit prototype current without relying on error-prone manual release steps.
...
2. **LOCKED:** Built async audit queues with Node.js, REST APIs, SQL, Redis, and BullMQ, keeping the Express API under 400 ms while long-running Playwright crawls ran in the background so one slow website could not block the remaining batch.

3. Automated service releases with Git and CI/CD, designed to keep audit logic current without relying on manual server updates.

#### SDET

1. Built automated audit-scoring tests with Jest, JavaScript, SQL, and functional testing, designed to apply the same checklist standards to every site review.

2. Validated REST API flows with Supertest, CI/CD, and Git, designed to catch broken submission and status workflows before a demonstration or review.

3. Built browser crawls with Playwright and accessibility testing so accessibility findings appear in the same site assessment instead of a separate tool.
