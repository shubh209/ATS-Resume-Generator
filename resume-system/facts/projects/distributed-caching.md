...
**RAG pipeline** — built a Python module with Zipf-distributed document retrieval, exact-set prefix matching, and a monthly cost projection. The model projected a 93.9% compute reduction and approximately $420/month lower compute cost at an assumed 1M daily requests on Llama-3-70B; it did not measure realized savings.
...
**RAG pipeline simulation (Python module)** — modeled a 93.9% compute reduction and projected approximately $420/month lower compute cost at an assumed 1M daily requests on the default configuration. This is not realized savings or production adoption.
...
### Backend SWE — Version 1 (current; locked September 2026)
...
**B2 — LOCKED**
Built a Go REST API with cache-aside PostgreSQL access and benchmarked four eviction policies across 10,000 operations, reaching a 98.7% cache-hit rate to reduce repeated database reads for frequently requested product data.
Keywords: [Go, REST API, Cache-Aside, PostgreSQL]
Metric type: MEASURED (source recorded in this master)
Business reason: Reduce repeated database reads for frequently requested product data.

**B3 — LOCKED**
Instrumented the Dockerized cache cluster with Prometheus and Grafana dashboards for throughput, latency, hit rate, and errors, making performance bottlenecks visible before selecting a cache policy or increasing cluster capacity.
Keywords: [Docker, Prometheus, Grafana, Observability]
Metric type: N/A
Business reason: Make performance bottlenecks visible before choosing a cache policy or increasing capacity.
...
**AI-2 — LOCKED**
Built a RAG pipeline simulation in Python with LangChain-style document retrieval and NumPy, modeling a 93.9% compute reduction and projecting approximately $420 per month in lower compute cost at an assumed 1M daily requests to compare infrastructure choices before budget is committed.
Keywords: [Python, RAG, LangChain, NumPy]
Metric type: [ESTIMATE/MODELED] — projected simulation result, not realized savings
